...
import { AgentInput, AgentOutput, JDAnalysis, UserProfile } from '../shared/types';
...
import { 
  ...
  KEYWORD_MAPPINGS 
} from '../../services/keywordNormalization';
...
Return full JD text and structured requirements as JSON.`;
  ...
  /**
   * Rule-based JD extraction fallback with semantic matching
   */
  private extractByRules(jdText: string): JDAnalysis {
    const textLower = jdText.toLowerCase();
    
    // Use semantic keyword extraction for better matching
    const semanticKeywords = extractNormalizedKeywords(jdText);
    
    // Also extract using traditional method for backup
    const technicalSkills = this.extractTechnicalSkills(textLower);
    const softSkills = this.extractSoftSkills(textLower);
    
    // Merge semantic and traditional extraction (semantic takes priority)
    const allTechnicalSkills = [...new Set([...semanticKeywords.filter(k => KEYWORD_MAPPINGS[k]), ...technicalSkills])];
    ...
    return {
      ...
      rawJD: jdText,
      requirements: {
        mustHave: allTechnicalSkills.slice(0, 5),
        preferred: allTechnicalSkills.slice(5, 8),
        experience,
        education: ['B.Tech', 'B.E.', 'BCA', 'MCA']
      },
      atsKeywords: [...allTechnicalSkills, ...softSkills].slice(0, 20),
  ...
  /**
   * Match JD keywords to resume with semantic understanding
   * Uses keyword normalization to find related terms
   */
  public matchKeywordsToResume(
    jdKeywords: string[], 
    resumeText: string
  ): { matched: string[]; missing: string[]; score: number } {
    return calculateSemanticMatchScore(jdKeywords, resumeText);
  }

  /**
   * Flatten resume data to searchable text
   */
  public flattenResumeToText(data: any): string {
    const parts: string[] = [];
    
    // Basics
    if (data.basics) {
      parts.push(data.basics.targetRole || '');
      parts.push(data.basics.fullName || '');
    }
    
    // Summary
    parts.push(data.summary || data.profile || '');
    
    // Experience
    if (data.experience && Array.isArray(data.experience)) {
      data.experience.forEach((exp: any) => {
        parts.push(exp.position || exp.role || '');
