// Unified LLM Client
// Single interface to call Groq, Gemini, and OpenAI with automatic fallback
...
const providerStatus: Record<LLMProvider, ProviderStatus> = {
  ...
  openai: { available: true, lastError: 0, errorCount: 0 }
};
...
function isProviderAvailable(provider: LLMProvider): boolean {
  ...
  switch (provider) {
    ...
    case 'openai':
      return status.available && !!OPENAI_API_KEY;
  }
}
...
export class LLMClient {
  ...
  hasBudget(provider: LLMProvider): boolean {
    switch (provider) {
      ...
      case 'openai':
        return this.budget.usedOpenAICalls < this.budget.maxOpenAICalls;
    }
  }
   ...
   */
  private getAvailableProvider(preferred: LLMProvider, isPaid: boolean): LLMProvider | null {
    const fallbackChain: LLMProvider[] = isPaid 
      ? ['groq', 'gemini', 'openai']
      : ['groq', 'gemini'];
  ...
  async call(
    ...
    const { preferred = 'groq', isPaid = false } = options;

    const provider = this.getAvailableProvider(preferred, isPaid);
    if (!provider) {
      throw new Error('No LLM providers available. Budget exhausted or all providers are down.');
    }
