...
/**
 * Generate authoritative template recommendations based on comprehensive analysis
 */
export async function generateAuthoritativeRecommendations(
    resume: {
        basics: any;
        summary: string;
        experience: any[];
    ...
    // Use the new API service for enterprise intelligence
    const apiService = await import('./api-service.js').then(module => module.ApiService);
    
    return await apiService.getTemplateRecommendations(resume as any, jobDescription);
}

// Helper function to estimate years from experience level
function estimateYearsFromExperienceLevel(level: string): number {
    switch(level) {
...
// Helper function to get template tradeoffs
function getTemplateTradeoffs(templateId: string): string[] {
    switch(templateId) {
        case 'tech':
...
/**
 * Get the top recommended template with full justification
 */
export async function getTopRecommendation(
    ...
    const result = await apiService.getTemplateRecommendations(resume, jobDescription);
    return result.recommendations[0]; // Return highest-ranked template
}
