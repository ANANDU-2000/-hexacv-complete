...
export function Hero({ onStart, onUpload, showFeedbackSuccess }: HeroProps) {
    ...
    return (
        <section className="relative w-full h-screen bg-white overflow-hidden flex flex-col">
            ...
            <div className="flex-1 flex flex-col lg:flex-row">
                ...
                <div className="hidden lg:flex w-1/2 h-full bg-gradient-to-br from-slate-50 to-slate-100 relative items-center justify-center p-8 overflow-hidden">
                    ...
                    <div className="relative w-full max-w-2xl">
                        ...
                        <div className="absolute -bottom-4 -right-4 bg-black text-white px-4 py-2 rounded-full shadow-xl flex items-center gap-2 animate-pulse">
                            <div className="w-2 h-2 bg-green-400 rounded-full"></div>
                            <span className="text-xs font-bold">ATS Optimized</span>
                        </div>
                    </div>
                </div>
            </div>

            {/* BOTTOM TESTIMONIAL ROW - Fixed, Auto-moving */}
            <div className="absolute bottom-0 left-0 right-0 bg-white border-t border-slate-100 py-4 overflow-hidden">
                    ...
                    {[...Array(2)].map((_, setIndex) => (
                        <div key={setIndex} className="flex items-center gap-12">
                            <div className="inline-flex items-center gap-4">
                                ...
                                <span className="font-black text-slate-900 text-xs">Software Dev, Bangalore</span>
                                <span className="text-slate-500 text-xs">"Finally passed the ATS filter after 3 months"</span>
                            </div>
                            <div className="inline-flex items-center gap-4">
                                <div className="flex gap-0.5">
                                    {[...Array(5)].map((_, i) => (
                                ...
                                <span className="font-black text-slate-900 text-xs">Product Manager, Delhi</span>
                                <span className="text-slate-500 text-xs">"No login needed - that's what I wanted"</span>
                            </div>
                            <div className="inline-flex items-center gap-4">
                                <div className="flex gap-0.5">
                                    {[...Array(5)].map((_, i) => (
                                ...
                                <span className="font-black text-slate-900 text-xs">Fresh Grad, Mumbai</span>
                                <span className="text-slate-500 text-xs">"Used it for my first job application"</span>
                            </div>
                            <div className="inline-flex items-center gap-4">
                                <div className="flex gap-0.5">
                                    {[...Array(5)].map((_, i) => (
                                ...
                                <span className="font-black text-slate-900 text-xs">Data Analyst, Hyderabad</span>
                                <span className="text-slate-500 text-xs">"Simple - just what I needed"</span>
                            </div>
                            <div className="inline-flex items-center gap-4">
                                <div className="flex gap-0.5">
                                    {[...Array(5)].map((_, i) => (
                                ...
                                <span className="font-black text-slate-900 text-xs">UX Designer, Pune</span>
                                <span className="text-slate-500 text-xs">"Clean PDF output, no watermarks"</span>
                            </div>
                        </div>
                    ))}
                </div>
            ...
            <div className="lg:hidden fixed inset-0 flex flex-col bg-[#F8F9FB] overflow-y-auto pt-[56px] pb-10 selection:bg-slate-900 selection:text-white">
                <style>{`
                    @keyframes float {
                ...
                <div className="flex-shrink-0 w-full flex flex-col items-center">
                    ...
                    <div className="w-full pb-10 flex justify-center px-6">
                        <div className="relative w-full max-w-[300px] animate-float">
                            ...
                            <div className="absolute -bottom-3 -right-2 bg-[#0F172A] text-white px-3.5 py-1.5 rounded-full shadow-lg flex items-center gap-1.5 border border-white/10 z-10">
                                ...
                                <span className="text-[10px] font-bold uppercase tracking-wider">ATS Optimized</span>
                            </div>
                        </div>
                    </div>
                    ...
                    {/* 4. Feedback / Testimonial Section */}
                    <div className="w-full pt-2 pb-10">
                        <div className="w-full overflow-x-auto flex gap-4 px-6 no-scrollbar snap-x">
                            {[
                                { name: "Rahul S.", text: "Got interview in 2 days with the ATS format", rating: 5 },
                                { name: "Anita M.", text: "Cleanest design I've found so far, love it.", rating: 5 },
                                { name: "John D.", text: "Finally an AI tool that actually helps.", rating: 5 },
                                { name: "Priya K.", text: "Layout is perfect for readability.", rating: 5 }
                            ].map((testimonial, i) => (
                                <div key={i} className="flex-shrink-0 w-[240px] snap-center bg-white p-5 rounded-2xl shadow-[0_4px_20px_-8px_rgba(0,0,0,0.06)] border border-[#F1F5F9] flex flex-col gap-3">
                                    <div className="flex items-center gap-2.5">
                                        ...
                                        <div>
                                            <p className="text-[13px] font-black text-[#111111] leading-none mb-1">{testimonial.name}</p>
                                            <div className="flex gap-0.5">
                                                {[...Array(5)].map((_, j) => (
                                                    <div key={j} className="w-1.5 h-1.5 rounded-full bg-slate-900"></div>
                                                ))}
                                            </div>
                                        </div>
                                    </div>
