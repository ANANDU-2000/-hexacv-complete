...
const SortableExperienceItem: React.FC<SortableExperienceItemProps> = ({ exp, idx, data, updateData, expandedItems, toggleExpand }) => {
...
const Step2Editor: React.FC<Step2EditorProps> = ({ data, onChange, onNext, onBack, validationErrors = [] }) => {
    ...
    // Search roles as user types - SMART RANKING with AI fallback for unknown roles
    const handleRoleSearch = async (input: string) => {
        if (!input || input.length === 0) {
            // Show popular/top roles when empty
            const popularRoles = [
                "AI Engineer", "Software Engineer", "Product Manager", "Data Scientist",
                "Full Stack Developer", "Marketing Manager", "UX Designer", "Sales Executive",
                "Financial Analyst", "Project Manager", "DevOps Engineer", "Business Analyst"
            ];
            setRoleSuggestions(popularRoles);
        ...
        const scoredRoles = COMMON_ROLES.map(role => {
            const roleLower = role.toLowerCase();
            let score = 0;

            // EXACT MATCH at start = highest priority (e.g., "AI Engineer" for "ai")
            if (roleLower.startsWith(inputLower)) {
                score += 1000;
            }

            // CONTAINS EXACT PHRASE (e.g., "AI Engineer" contains "ai engin")
            if (roleLower.includes(inputLower)) {
                score += 500;
            }

            // ALL WORDS MATCH (order-independent)
            const allWordsMatch = searchTerms.every(term => roleLower.includes(term));
            if (allWordsMatch) {
                score += 300;
            }

            // PARTIAL WORD MATCHES
            const matchedWords = searchTerms.filter(term => roleLower.includes(term)).length;
            score += matchedWords * 50;

            // WORD BOUNDARY MATCH (e.g., "AI" at word start)
            searchTerms.forEach(term => {
                if (roleLower.split(/\s+/).some(word => word.startsWith(term))) {
                    score += 100;
                }
            });
        ...
        // Sort by score (descending) and filter only those with score > 0
        let matches = scoredRoles
            .filter(item => item.score > 0)
            .sort((a, b) => b.score - a.score)
            .slice(0, 10)
            .map(item => item.role);

        // If no matches found, allow user's input as valid role (with capitalization fix)
        if (matches.length === 0) {
            ...
            // Add as suggestion with note that it's custom
            matches = [
                capitalizedInput + " (Custom Role)",
                "Software Engineer",
                "Product Manager",
                "Project Manager"
            ];
