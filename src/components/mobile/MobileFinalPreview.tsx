...
import {
    X, ZoomIn, ZoomOut, ChevronLeft, ChevronRight, Download, Lock, Eye, Search, RotateCcw
} from 'lucide-react';
...
const MobileFinalPreview: React.FC<MobileFinalPreviewProps> = ({ data, selectedTemplate, onBack, onNext }) => {
    ...
    const [zoom, setZoom] = useState(0.42); // Reduced default zoom for better mobile fit
    ...
    const handleZoomIn = () => setZoom(prev => Math.min(1.0, prev + 0.08));
    const handleZoomOut = () => setZoom(prev => Math.max(0.25, prev - 0.08));
    ...
    const handleTouchEnd = (e: React.TouchEvent) => {
        ...
        if (Math.abs(deltaX) > Math.abs(deltaY) && Math.abs(deltaX) > 50) {
            if (deltaX < 0 && currentPage < pageCount) {
                                ...
                                onClick={async () => {
                                    try {
                                        ...
                                        if (result.success && result.templateId) {
                                            await handlePaymentSuccess(result.templateId);
                                        }
                                    } catch (error) {
    ...
    return (
        ...
        `}>
            {/* iOS Style Header */}
            <div className="flex-none pt-10 pb-3 px-4 bg-[#F5F5F7]/80 backdrop-blur-xl border-b border-gray-200/50 flex items-center justify-between">
                <button
                    onClick={onBack}
                    className="w-9 h-9 flex items-center justify-center rounded-full active:bg-black/5 transition-all text-black"
                >
                    <ChevronLeft size={20} strokeWidth={2.5} />
                </button>

                <div className="flex flex-col items-center flex-1">
                    <span className="text-[9px] font-black text-gray-400 uppercase tracking-[0.15em] mb-0.5">Final Preview</span>
                    <span className="text-xs font-black text-black uppercase tracking-tight">{selectedTemplate.name}</span>
                </div>

                <div className="bg-black px-2.5 py-1 rounded-lg shadow-sm">
            ...
            <div
                ...
                className="flex-1 relative overflow-y-auto overflow-x-hidden p-6 pb-40 flex flex-col items-center gap-8 select-none"
            >
            ...
            {/* Floating Zoom Bar - iOS Style - Compact */}
            <div className="fixed bottom-32 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 bg-white/90 backdrop-blur-2xl px-4 py-3 rounded-2xl shadow-xl border border-gray-100">
                <button onClick={handleZoomOut} className="text-black/30 active:text-black transition-colors p-1">
                    <ZoomOut size={18} strokeWidth={2.5} />
                </button>
                <div className="w-px h-3 bg-gray-100" />
                <span className="text-[10px] font-black text-black min-w-[2.5rem] text-center uppercase tracking-widest">
                    {Math.round(zoom * 100)}%
                </span>
                <div className="w-px h-3 bg-gray-100" />
                <button onClick={handleZoomIn} className="text-black/30 active:text-black transition-colors p-1">
                    <ZoomIn size={18} strokeWidth={2.5} />
                </button>
                <div className="w-px h-3 bg-gray-100" />
                <button onClick={handleFitToWidth} className="text-black/30 active:text-black transition-colors p-1 flex items-center gap-1">
                    <RotateCcw size={14} strokeWidth={2.5} />
                    <span className="text-[8px] font-black uppercase tracking-wider">Fit</span>
                </button>
            </div>

            {/* Bottom Actions - Monochromatic Premium - Compact */}
            <div className="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-xl p-4 pb-8 border-t border-gray-100 safe-area-bottom space-y-3">
                <button
                    onClick={() => setShowStep3Preview(true)}
                    className="w-full h-12 rounded-xl bg-gray-50 text-gray-400 font-black text-[10px] uppercase tracking-[0.15em] active:scale-[0.98] transition-all flex items-center justify-center gap-2"
                >
                    <Eye size={16} strokeWidth={2.5} />
                    <span>Focus Mode Preview</span>
                </button>
                ...
                <button
                    ...
                    disabled={processing}
                    className={`
                        w-full h-14 rounded-xl flex items-center justify-center gap-2 border-2 active:scale-[0.98] transition-all shadow-lg shadow-black/10
                        ${unlocked ? 'bg-white text-black border-black' : 'bg-white text-black border-black'}
