...
import {
    X, ZoomIn, ZoomOut, ChevronLeft, ChevronRight, Download, Lock, Eye, Search
} from 'lucide-react';
...
const MobileFinalPreview: React.FC<MobileFinalPreviewProps> = ({ data, selectedTemplate, onBack, onNext }) => {
    ...
    return (
        ...
        `}>
            {/* iOS Style Header */}
            <div className="flex-none pt-12 pb-4 px-6 bg-[#F5F5F7]/80 backdrop-blur-xl border-b border-gray-200/50 flex items-center justify-between">
                <button
                    onClick={onBack}
                    className="w-10 h-10 flex items-center justify-center rounded-full active:bg-black/5 transition-all text-black"
                >
                    <ChevronLeft size={22} strokeWidth={2.5} />
                </button>

                <div className="flex flex-col items-center flex-1">
                    <span className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-0.5">Final Preview</span>
                    <span className="text-sm font-black text-black uppercase tracking-tight">{selectedTemplate.name}</span>
                </div>

                <div className="bg-black px-3 py-1.5 rounded-xl shadow-sm">
            ...
            {/* Floating Zoom Bar - iOS Style */}
            <div className="fixed bottom-36 left-1/2 -translate-x-1/2 z-50 flex items-center gap-4 bg-white/90 backdrop-blur-2xl px-6 py-4 rounded-[2rem] shadow-2xl border border-gray-100">
                <button onClick={handleZoomOut} className="text-black/30 active:text-black transition-colors">
                    <ZoomOut size={20} strokeWidth={2.5} />
                </button>
                <div className="w-px h-4 bg-gray-100" />
                <span className="text-[11px] font-black text-black min-w-[3rem] text-center uppercase tracking-widest">
                    {Math.round(zoom * 100)}%
                </span>
                <div className="w-px h-4 bg-gray-100" />
                <button onClick={handleZoomIn} className="text-black/30 active:text-black transition-colors">
                    <ZoomIn size={20} strokeWidth={2.5} />
                </button>
            </div>

            {/* Bottom Actions - Monochromatic Premium */}
            <div className="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-xl p-6 pb-10 border-t border-gray-100 safe-area-bottom space-y-4">
                <button
                    onClick={() => setShowStep3Preview(true)}
                    className="w-full h-14 rounded-[1.5rem] bg-gray-50 text-gray-400 font-black text-[11px] uppercase tracking-[0.2em] active:scale-[0.98] transition-all flex items-center justify-center gap-3"
                >
                ...
                <button
                    ...
                    disabled={processing}
                    className={`
                        w-full h-16 rounded-[2rem] flex items-center justify-center gap-3 border-2 active:scale-[0.98] transition-all shadow-xl shadow-black/10
