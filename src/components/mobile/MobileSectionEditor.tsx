...
import { ChevronLeft, ChevronUp, ChevronDown, Plus, Trash2, GripVertical, User, FileText, Building2, Briefcase, Calendar, Code, GraduationCap, Award, ChevronRight, Image as ImageIcon, CheckCircle2, Check, X } from 'lucide-react';
        ...
        return (
            <div className="h-[100dvh] flex flex-col bg-[#F5F5F7] font-sans">
                ...
                <div className="flex-1 overflow-y-auto px-6 pt-24 pb-32 scrollbar-hide">
                    ...
                    <div className="mb-10 bg-white p-8 rounded-[2.5rem] border border-white shadow-[0_4px_20px_rgba(0,0,0,0.03)]">
                        <div className="flex items-center gap-6">
                            <div className="relative">
                                {photoPreview ? (
                                    <div className="relative">
                                        <img src={photoPreview} alt="Profile" className="w-24 h-24 rounded-[2rem] object-cover ring-4 ring-gray-50 shadow-md" />
                                        <button onClick={() => { setPhotoPreview(null); onChange({ photoUrl: undefined, includePhoto: false }); }} className="absolute -top-2 -right-2 w-8 h-8 bg-black text-white rounded-full flex items-center justify-center shadow-lg active:scale-90 transition-all font-black">
                                            <Trash2 size={14} strokeWidth={3} />
                                        </button>
                                    </div>
                                ) : (
                                    <label className="w-24 h-24 rounded-[2rem] border-2 border-dashed border-gray-100 bg-gray-50/50 flex flex-col items-center justify-center cursor-pointer active:scale-95 transition-all shadow-inner">
                                        ...
                                        <span className="text-[10px] font-black text-gray-300 uppercase tracking-widest leading-none">Add Icon</span>
                                    </label>
                                )}
                            </div>
                            <div className="flex-1">
                                <p className="text-[11px] font-black text-gray-400 uppercase tracking-[0.2em] mb-1">Profile Image</p>
                                <p className="text-[13px] text-black font-black uppercase tracking-tight">Professional Headshot</p>
                                {photoPreview && (
                                    <label className="flex items-center gap-2.5 mt-4 cursor-pointer">
                                        <div className="relative w-5 h-5">
                                            <input type="checkbox" checked={data.includePhoto || false} onChange={(e) => onChange({ includePhoto: e.target.checked })} className="peer appearance-none w-5 h-5 rounded-lg border-2 border-gray-100 checked:bg-black checked:border-black transition-all cursor-pointer" />
                                        ...
                                        <span className="text-[11px] font-black text-black uppercase tracking-wider">Include</span>
                                    </label>
                                )}
                            </div>
                        </div>
