...
const MobileTemplateExport: React.FC<MobileTemplateExportProps> = ({
    ...
    return (
        <div className="h-[100dvh] flex flex-col bg-[#F8F9FB] overflow-hidden select-none font-sans">
            ...
            {showFullscreen && (
                <div className="fixed inset-0 z-[1000] bg-[#F5F5F7] flex flex-col animate-in fade-in duration-300">
                    {/* Full Resume View - No Header */}
                    <div className="flex-1 overflow-auto p-4 flex justify-center items-start">
                        <div className="origin-top my-4 shadow-2xl bg-white" style={{ width: '210mm', minHeight: '297mm', transform: 'scale(0.38)' }}>
                            <TemplateRenderer
                    ...
                    {/* Minimal Bottom Bar */}
                    <div className="flex-none px-4 py-3 bg-white/95 backdrop-blur-xl border-t border-gray-200/60 safe-area-bottom flex items-center justify-between">
                        <button
                        ...
                        <button
                            onClick={() => handleDownload(selectedTemplate)}
                            className="px-6 py-2.5 bg-black text-white rounded-xl font-bold text-sm active:scale-95 transition-all flex items-center gap-2"
                        >
                            <Download size={16} />
                            <span>Download</span>
                        </button>
