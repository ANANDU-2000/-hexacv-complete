...
/**
 * Enhanced fallback parsing using regex patterns (no AI needed)
 */
function fallbackParse(text: string): ExtractionResult {
    ...
    for (const pattern of phonePatterns) {
        ...
        if (match) { phone = match[0].replace(/[^\d+]/g, ''); break; }
    }
    ...
    // Extract name (first non-empty line, cleaned)
    const lines = text.split('\n').map(l => l.trim()).filter(l => l.length > 0 && l.length < 100);
    let name = lines[0] || '';
    // Remove common prefixes
    name = name.replace(/^(name|resume|cv|curriculum vitae)[:\s]*/i, '').trim();
    if (name) extractedFields.push('Name'); else missingFields.push('Name');

    // Extract skills (look for common skill keywords)
    const skillKeywords = ['javascript', 'typescript', 'python', 'java', 'react', 'node', 'sql', 
        'aws', 'docker', 'kubernetes', 'git', 'html', 'css', 'c++', 'c#', 'go', 'rust',
        'mongodb', 'postgresql', 'mysql', 'redis', 'graphql', 'rest', 'api', 'agile',
    ...
    if (foundSkills.length > 0) extractedFields.push('Skills'); else missingFields.push('Skills');

    // Extract experience sections (look for date patterns near company-like text)
    const expPattern = /(\d{4})\s*[-–]\s*(present|\d{4})/gi;
    const expMatches = text.match(expPattern);
    if (expMatches && expMatches.length > 0) extractedFields.push('Experience'); else missingFields.push('Experience');

    // Calculate confidence based on extraction success
    const confidence = Math.round((extractedFields.length / (extractedFields.length + missingFields.length)) * 100);
    ...
    return {
        resume: {
            ...
            github,
            profile: '',
            skills: foundSkills.length > 0 ? [{ category: 'Skills', items: foundSkills }] : [],
            experience: [],
            education: [],
            projects: []
        },
