...
export const ExperienceSection: React.FC<ExperienceSectionProps> = ({ data, onChange }) => {
  ...
  const itemIds = experiences.map((e, i) => e.id || `exp-${i}`);
...
type ExperienceItem = ResumeData['experience'] extends (infer E)[] ? E : never;
...
function ExperienceSortableItem({
  ...
  onUpdateHighlight: (expIndex: number, bulletIndex: number, value: string) => void;
      ...
      {isExpanded && (
        <div className="border-t border-gray-200 p-4 space-y-3">
          ...
          <div className="border-t border-gray-200 pt-3 mt-2">
            <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
              ...
              <span className="flex gap-2">
                <button type="button" onClick={() => onAddBullet(index)} className="text-sm px-2 py-1.5 text-blue-600 hover:bg-blue-50 rounded min-h-[44px]">+ Add bullet</button>
              ...
              {(exp.highlights || []).map((bullet, bIdx) => (
                <li key={bIdx} className="flex gap-2 items-start">
                  ...
                  <input
                    ...
                    value={bullet}
                    onChange={(e) => onUpdateHighlight(index, bIdx, e.target.value)}
                    className="flex-1 border border-gray-300 p-2 rounded text-sm"
                  />
                  <button
                    type="button"
                    onClick={() => onRemoveBullet(index, bIdx)}
          ...
          <div className="flex justify-end pt-2">
            <button type="button" onClick={() => onRemove(index)} className="text-sm text-gray-500 hover:text-red-600 min-h-[44px] px-2">Remove experience</button>
          </div>
        </div>
      )}
    </div>
